...
import { exportToCSV } from '@/components/ui/SmartTableToolbar';
...
function StatusChip({ status }: { status: StatusType }) {
  const configs: Record<StatusType, { label: string; color: string; bg: string; border: string; dot: string }> = {
    healthy:   { label: 'Healthy',   color: '#22C55E', bg: 'rgba(34,197,94,0.12)',   border: 'rgba(34,197,94,0.2)',   dot: '#22C55E' },
    running:   { label: 'Running',   color: '#38BDF8', bg: 'rgba(56,189,248,0.12)',  border: 'rgba(56,189,248,0.2)',  dot: '#38BDF8' },
    completed: { label: 'Completed', color: '#22C55E', bg: 'rgba(34,197,94,0.12)',   border: 'rgba(34,197,94,0.2)',   dot: '#22C55E' },
    failed:    { label: 'Failed',    color: '#EF4444', bg: 'rgba(239,68,68,0.12)',   border: 'rgba(239,68,68,0.2)',   dot: '#EF4444' },
    pending:   { label: 'Pending',   color: '#94A3B8', bg: 'rgba(148,163,184,0.12)', border: 'rgba(148,163,184,0.2)', dot: '#94A3B8' },
    warning:   { label: 'Warning',   color: '#F59E0B', bg: 'rgba(245,158,11,0.12)',  border: 'rgba(245,158,11,0.2)',  dot: '#F59E0B' },
    critical:  { label: 'Critical',  color: '#EF4444', bg: 'rgba(239,68,68,0.12)',   border: 'rgba(239,68,68,0.2)',   dot: '#EF4444' },
    offline:   { label: 'Offline',   color: '#64748B', bg: 'rgba(100,116,139,0.12)', border: 'rgba(100,116,139,0.2)', dot: '#64748B' },
    paused:    { label: 'Paused',    color: '#F59E0B', bg: 'rgba(245,158,11,0.12)',  border: 'rgba(245,158,11,0.2)',  dot: '#F59E0B' },
  };
      ...
      <div className="relative">
        ...
        {notifOpen && (
          ...
          >
            ...
            {[
              { title: 'Migration Failed', detail: 'legacy-mssql-migration encountered a constraint error', time: '7m ago', color: 'var(--akaal-error)', href: '/execution-center?status=failed' },
              { title: 'CDC Lag Warning', detail: 'prod-oracle-to-postgres replication delay: 4.2s', time: '23m ago', color: 'var(--akaal-warning)', href: '/live-monitor' },
              { title: 'Approval Required', detail: 'crm-postgres-upgrade awaiting production sign-off', time: '1h ago', color: 'var(--akaal-secondary)', href: '/execution-center?job=m4' },
            ].map((n, i) => (
              <Link key={i} href={n.href} onClick={() => setNotifOpen(false)} className="px-3 py-2.5 transition-colors block cursor-pointer" style={{ borderBottom: i < 2 ? '1px solid var(--akaal-border-subtle)' : 'none' }}
...
export default function DashboardPage() {
  ...
  const filteredMigrations = MOCK_MIGRATIONS
    ...
    .filter(m => {
      ...
      return m.name.toLowerCase().includes(q) || m.source.toLowerCase().includes(q) || m.target.toLowerCase().includes(q) || m.owner.toLowerCase().includes(q);
    })
    .sort((a, b) => {
  ...
  const metrics: MetricCard[] = [
    {
      ...
      accentColor: '#38BDF8',
      href: '/execution-center?status=running',
    ...
    },
    {
      ...
      accentColor: '#EF4444',
      href: '/execution-center?status=failed',
    ...
    },
    {
      ...
      accentColor: '#F59E0B',
      href: '/execution-center?job=EX-2850',
  ...
  const healthItems: HealthItem[] = [
    { id: 'sys', label: 'System Status', status: 'healthy', detail: 'All services operational', href: '/system' },
    { id: 'agent', label: 'Agent Health', status: 'warning', detail: '2 agents degraded', href: '/agents' },
    { id: 'cdc', label: 'CDC Status', status: 'warning', detail: 'Replication lag: 4.2s', href: '/live-monitor' },
    { id: 'queue', label: 'Queue Health', status: 'healthy', detail: '0 stalled jobs', href: '/execution-center' },
    { id: 'db', label: 'DB Connectivity', status: 'healthy', detail: '14/14 connected', href: '/databases' },
    { id: 'storage', label: 'Storage Usage', status: 'healthy', detail: '62% utilized', href: '/system' },
  ];
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button
                  type="button"
                  onClick={() => {
                    exportToCSV('dashboard-summary', MOCK_MIGRATIONS);
                  }}
                ...
                {isLoading
                  ...
                  : metrics.map(m => (
                    <Link
                      key={m.id}
                      href={m.href}
                      className="akaal-card-kpi block cursor-pointer group"
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
                      {healthItems.map(item => (
                        <Link
                          key={item.id}
                          href={item.href}
                          className="rounded-md p-3 transition-all duration-150 block cursor-pointer group"
                        ...
                        >
                          <p className="text-xs mb-2 group-hover:text-white transition-colors" style={{ color: 'var(--akaal-text-muted)', fontFamily: "'Inter', sans-serif" }}>{item.label}</p>
                  ...
                  action={
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="text-xs rounded-md px-2.5 py-1 outline-none transition-all duration-150"
                        ...
                        aria-label="Search recent migrations"
                      />
                      {/* Status filter */}
                      <select
                        value={statusFilter}
                ...
                {isLoading ? (
                  ...
                  <div className="p-6">
                    <EnterpriseEmptyState
                      title={statusFilter !== 'all' ? `No ${statusFilter} migrations found` : 'No active migrations configured'}
                      description={statusFilter !== 'all' ? `No migrations currently match the selected status '${statusFilter}'. Reset the filter to view all migrations.` : 'There are no active or scheduled database migrations in this environment. Launch a new migration workspace to begin.'}
                      primaryAction={statusFilter !== 'all' ? { label: 'Reset Filter', onClick: () => setStatusFilter('all') } : { label: 'Create Migration', href: '/migration-workspace' }}
                      secondaryAction={statusFilter !== 'all' ? { label: 'Create Migration', href: '/migration-workspace' } : undefined}
                    />
                  </div>
                ) : (

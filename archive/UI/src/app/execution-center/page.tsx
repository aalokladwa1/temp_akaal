    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
      {cards.map((card, i) => (
        <div
          ...
          style={{ background: '#1F2937', border: '1px solid #2A3647' }}
        >
          ...
          <div className="text-xl font-bold tabular-nums" style={{ color: '#F8FAFC', fontFamily: "'JetBrains Mono', monospace", lineHeight: 1.1 }}>{card.value}</div>
          <div className="text-xs mt-1 font-medium" style={{ color: '#CBD5E1', fontFamily: "'Inter', sans-serif" }}>{card.label}</div>
          <div className="text-xs mt-0.5" style={{ color: '#64748B', fontFamily: "'Inter', sans-serif" }}>{card.sub}</div>
        </div>
      ))}
    </div>
  );
...
function FilterPanel({
  ...
  return (
    <aside
      ...
      style={{ width: '192px', background: '#111827', borderRight: '1px solid #2A3647' }}
      aria-label="Execution filters"
    >
      <div className="px-3 py-3 flex-shrink-0 flex items-center justify-between" style={{ borderBottom: '1px solid #2A3647' }}>
...
function ExecutionTable({
  ...
  if (sorted.length === 0) {
    return (
      <div className="rounded-lg flex flex-col items-center justify-center py-16" style={{ background: '#1F2937', border: '1px solid #2A3647' }}>
        <div className="w-10 h-10 rounded-lg flex items-center justify-center mb-3" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid #2A3647' }}>
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" aria-hidden="true" style={{ color: '#64748B' }}>
            <circle cx="10" cy="10" r="8" stroke="currentColor" strokeWidth="1.5" />
            <path d="M7 10h6M10 7v6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
          </svg>
        </div>
        <p className="text-sm font-medium" style={{ color: '#CBD5E1', fontFamily: "'Inter', sans-serif" }}>No executions found</p>
        <p className="text-xs mt-1" style={{ color: '#64748B', fontFamily: "'Inter', sans-serif" }}>Adjust your filters or launch a new migration</p>
      </div>
    );
  }
...
function DetailWorkspace({
  ...
  return (
    <div
      ...
      style={{ width: '520px', background: '#111827', borderLeft: '1px solid #2A3647', overflow: 'hidden' }}
      aria-label="Execution detail workspace"
    >
      {/* Header */}
      <div className="flex items-start justify-between px-4 py-3 flex-shrink-0" style={{ borderBottom: '1px solid #2A3647' }}>
        <div className="flex-1 min-w-0 mr-3">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-bold tabular-nums" style={{ color: '#2563EB', fontFamily: "'JetBrains Mono', monospace" }}>{job.id}</span>
            <ExecStatusChip status={job.status} />
            <PriorityChip priority={job.priority} />
          </div>
          <p className="text-sm font-semibold truncate" style={{ color: '#F8FAFC', fontFamily: "'Inter', sans-serif" }}>{job.migrationName}</p>
          <p className="text-xs mt-0.5" style={{ color: '#64748B', fontFamily: "'Inter', sans-serif" }}>{job.source} → {job.target}</p>
        </div>
        <button
          type="button"
          onClick={onClose}
      ...
      {/* Job Controls */}
      <div className="flex items-center gap-1.5 px-4 py-2.5 flex-shrink-0 flex-wrap" style={{ borderBottom: '1px solid #2A3647', background: '#0F1929' }}>
        {canPause && (
          <button type="button" onClick={() => onAction('pause', job)}

  ...
  template: `
    <div class="flex flex-col gap-6 w-full max-w-[1680px] mx-auto font-sans pb-16 select-none animate-in fade-in duration-150" *ngIf="rule(); else notFound">
      ...
      <div class="flex items-start justify-between gap-6 pb-5 border-b border-slate-200 flex-wrap">
        ...
        <div class="flex items-center gap-2 pt-1">
          <a
            [routerLink]="['/administration/governance-centre/sod', rule()?.id, 'edit']"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-white border border-slate-200 rounded-xl p-6 flex flex-col gap-4 shadow-2xs">
          <h2 class="text-base font-bold text-slate-900 font-heading border-b border-slate-100 pb-3">Incompatible Pair</h2>
          <div class="grid grid-cols-1 gap-3 text-xs">
            <div class="flex flex-col gap-1">
              <span class="text-slate-500 font-medium">Role A</span>
              <span class="font-mono font-semibold text-rose-700">{{ rule()?.incompatibleRoleA }}</span>
            </div>
            <div class="flex flex-col gap-1">
              <span class="text-slate-500 font-medium">Role B</span>
              <span class="font-mono font-semibold text-rose-700">{{ rule()?.incompatibleRoleB }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <ng-template #notFound>
      <div class="p-8 text-center text-slate-500 text-sm">
        SoD rule not found. <a routerLink="/administration/governance-centre/sod" class="text-blue-600 underline">Back</a>
      </div>
    </ng-template>
  `
})
export class SodDetailComponent {
  ...
  public rule = computed(() => this.govService.sodRules().find(r => r.id === this.rId()));
}
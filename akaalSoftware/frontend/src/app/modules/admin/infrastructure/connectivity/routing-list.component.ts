/**
 * AKAAL Administration — 5.7 Network Routing & Bastions List
 */
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-start justify-between gap-6 pb-5 border-b border-slate-200 flex-wrap">
          <div class="flex flex-col gap-1">
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight font-heading">Network Routing &amp; Bastions</h1>
            <p class="text-sm font-medium text-slate-600 max-w-3xl">
              Configured enterprise forward proxies, reverse ingress gateways, and SSH bastion jump hosts.
            </p>
          </div>

          <div class="flex items-center gap-2">
            <a
              routerLink="/administration/infrastructure/connectivity/routing/create"
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="border-b border-slate-200 bg-slate-50/75 text-[11px] font-bold text-slate-600 uppercase tracking-wider font-heading">
              ...
              <th class="py-3 px-4">Target Host Endpoint</th>
          ...
          <tbody class="divide-y divide-slate-100 text-xs font-medium text-slate-700">
            @for (item of service.networkRoutes(); track item.id) {
              <tr class="hover:bg-slate-50/80 transition-colors">
                ...
                <td class="py-3.5 px-4 font-mono text-slate-700">{{ item.targetHost }}</td>
                <td class="py-3.5 px-4 font-mono text-slate-800 font-bold">{{ item.targetPort }}</td>
                <td class="py-3.5 px-4 font-mono text-[11px] text-slate-500 truncate max-w-xs">{{ item.credentialRef || 'None (Anonymous)' }}</td>
                <td class="py-3.5 px-4">

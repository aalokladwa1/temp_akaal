/**
 * AKAAL Administration — 5.7 Execution Sites List
 */
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-start justify-between gap-6 pb-5 border-b border-slate-200 flex-wrap">
          <div class="flex flex-col gap-1">
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight font-heading">Execution Sites</h1>
            <p class="text-sm font-medium text-slate-600 max-w-3xl">
              Physical and virtual deployment locations where migration worker processes are scheduled and executed.
            </p>
          </div>

          <div class="flex items-center gap-2">
            <a
              routerLink="/administration/infrastructure/compute/sites/create"
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="border-b border-slate-200 bg-slate-50/75 text-[11px] font-bold text-slate-600 uppercase tracking-wider font-heading">
              ...
              <th class="py-3 px-4">Provider / Datacenter</th>
              <th class="py-3 px-4">Placement Region</th>
              <th class="py-3 px-4">Max Parallel Jobs</th>
              <th class="py-3 px-4">Status</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100 text-xs font-medium text-slate-700">
            @for (item of service.executionSites(); track item.id) {
              <tr class="hover:bg-slate-50/80 transition-colors">
                ...
                <td class="py-3.5 px-4 font-mono text-[11px] font-semibold text-slate-800">{{ item.siteType }}</td>
                <td class="py-3.5 px-4 text-slate-700">{{ item.providerOrDatacenter }}</td>
                <td class="py-3.5 px-4 font-mono text-slate-600">{{ item.region }}</td>
                <td class="py-3.5 px-4 font-mono font-bold text-slate-800">{{ item.maxParallelJobs }} workers</td>

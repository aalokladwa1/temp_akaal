/**
 * AKAAL Administration — Boundaries List
 */
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-start justify-between gap-6 pb-5 border-b border-slate-200 flex-wrap">
          <div class="flex flex-col gap-1">
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight font-heading">Resource Boundaries</h1>
            <p class="text-sm font-medium text-slate-600 max-w-3xl">
              Logical partitions, network perimeter policies, and data containment boundaries.
            </p>
          </div>

          <div class="flex items-center gap-2 pt-1">
            <a
              routerLink="/administration/enterprise/boundaries/create"
      ...
      <div class="flex items-center justify-between gap-4 flex-wrap bg-white p-3.5 border border-slate-200 rounded-xl shadow-2xs">
        <div class="relative flex-1 min-w-[240px] max-w-md">
          ...
          <input
            type="text"
            [(ngModel)]="searchQuery"
            placeholder="Search boundaries by name or code..."
        ...
        <div class="text-xs text-slate-500 font-medium">
          Showing <span class="font-bold text-slate-900">{{ filteredBoundaries().length }}</span> of {{ enterprise.boundaries().length }} Boundaries
        </div>
      </div>
        ...
        <table class="w-full text-left text-xs border-collapse">
          <thead>
            <tr class="border-b border-slate-200 bg-slate-50/75 text-[11px] font-bold text-slate-500 uppercase tracking-wider font-heading">
              <th class="py-3 px-4">Boundary Name</th>
              <th class="py-3 px-4">Code</th>
              <th class="py-3 px-4">Workspace</th>
              <th class="py-3 px-4">Isolation Policy</th>
          ...
          <tbody class="divide-y divide-slate-100 font-sans">
            <tr *ngFor="let b of filteredBoundaries()" class="hover:bg-slate-50/60 transition-colors">
              <td class="py-3 px-4 font-bold text-slate-900">
                <a [routerLink]="['/administration/enterprise/boundaries', b.id]" class="hover:text-blue-600 transition-colors">
                  {{ b.name }}
                </a>
              </td>
              <td class="py-3 px-4 font-mono font-medium text-slate-700">{{ b.code }}</td>
              <td class="py-3 px-4 text-slate-700 font-medium">{{ b.workspaceName }}</td>
              <td class="py-3 px-4 font-mono text-slate-700">{{ b.isolationPolicy }}</td>
              <td class="py-3 px-4">
              ...
              <td class="py-3 px-4 font-semibold text-slate-800">{{ b.boundConnectionsCount }}</td>
              <td class="py-3 px-4 text-right">
                <a
                  [routerLink]="['/administration/enterprise/boundaries', b.id]"
...
export class BoundaryListComponent {
  ...
  public searchQuery = '';
  public selectedClassification = 'ALL';
  ...
  public filteredBoundaries() {
    return this.enterprise.boundaries().filter(b => {
      const matchesSearch = !this.searchQuery ||
        b.name.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
        b.code.toLowerCase().includes(this.searchQuery.toLowerCase());
      const matchesClass = this.selectedClassification === 'ALL' || b.dataClassification === this.selectedClassification;

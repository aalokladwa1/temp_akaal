/**
 * AKAAL Administration — Create Organization
 */
    ...
    <div class="flex flex-col gap-6 w-full max-w-3xl mx-auto font-sans py-4 pb-16 select-none animate-in fade-in duration-150">
      ...
      <div class="flex items-start justify-between gap-6 pb-5 border-b border-slate-200">
        <div class="flex flex-col gap-1">
          <h1 class="text-2xl font-bold text-slate-900 tracking-tight font-heading">Create Organization</h1>
          <p class="text-sm font-medium text-slate-600 max-w-2xl">
            Provision a top-level holding entity, regional subsidiary, or business operating unit.
          </p>
        </div>
      </div>
      ...
      <div class="bg-white border border-slate-200 rounded-xl shadow-2xs p-6 flex flex-col gap-6 w-full">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="flex flex-col gap-1.5">
            ...
            <input
              type="text"
              [(ngModel)]="name"
              placeholder="e.g. Akaal Capital Markets EMEA"
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <input
              ...
              placeholder="e.g. s.jenkins@akaaltech.corp"
              class="h-9 px-3 text-xs rounded border border-slate-200 bg-slate-50/50 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white transition-colors"
            />
          </div>
          ...
          <div class="flex flex-col gap-1.5 md:col-span-2">
            ...
            <textarea
              [(ngModel)]="description"
              rows="3"
              placeholder="Describe the operational scope and regulatory boundaries of this organization..."
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
          <a
            routerLink="/administration/enterprise/organizations"
...
export class OrgCreateComponent {
  ...
  public save() {
    ...
    const newOrg = this.enterprise.createOrganization({
      name: this.name,
      code: this.code,
      description: this.description,
      tier: this.tier,
      primaryContactName: this.primaryContactName || 'Admin Lead',
      primaryContactEmail: this.primaryContactEmail || 'admin@akaaltech.corp',
      costCenterCode: this.costCenterCode || 'CC-DEFAULT'
    });
    this.router.navigate(['/administration/enterprise/organizations', newOrg.id]);
  }
}

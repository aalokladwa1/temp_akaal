/**
 * AKAAL Administration — Create Workspace
 */
    ...
    <div class="flex flex-col gap-6 w-full max-w-3xl mx-auto font-sans py-4 pb-16 select-none animate-in fade-in duration-150">
      ...
      <div class="flex items-start justify-between gap-6 pb-5 border-b border-slate-200">
        <div class="flex flex-col gap-1">
          <h1 class="text-2xl font-bold text-slate-900 tracking-tight font-heading">Create Workspace</h1>
          <p class="text-sm font-medium text-slate-600 max-w-2xl">
            Provision an isolated workspace container for migration initiatives and pipeline orchestration.
          </p>
        </div>
      </div>
      ...
      <div class="bg-white border border-slate-200 rounded-xl shadow-2xs p-6 flex flex-col gap-6 w-full">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-bold text-slate-700 uppercase tracking-wider font-heading">Parent Organization</label>
            <app-custom-select
              [options]="orgOptions()"
              [(ngModel)]="orgId">
            </app-custom-select>
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <input
              type="text"
              [(ngModel)]="name"
              placeholder="e.g. Payments Modernization 2026"
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <input
              type="email"
              [(ngModel)]="ownerEmail"
              placeholder="e.g. aalok.ladwa@akaaltech.internal"
              class="h-9 px-3 text-xs rounded border border-slate-200 bg-slate-50/50 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white transition-colors"
            />
          </div>
          ...
          <div class="flex flex-col gap-1.5 md:col-span-2">
            ...
            <textarea
              [(ngModel)]="description"
              rows="3"
              placeholder="Describe the initiative and scope for this workspace..."
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
          <a
            routerLink="/administration/enterprise/workspaces"
...
export class WorkspaceCreateComponent {
  ...
  public ownerName = '';
  public ownerEmail = '';
  public residencyRegion = 'us-east-1';
  ...
  public orgOptions = computed<CustomSelectOption[]>(() => {
    return this.enterprise.organizations().map(org => ({
  ...
  constructor() {
    const orgs = this.enterprise.organizations();
    if (orgs.length > 0) {
  ...
  public save() {
    if (!this.name || !this.code || !this.orgId) return;
    const newWs = this.enterprise.createWorkspace({
      orgId: this.orgId,
      name: this.name,
      code: this.code,
      tier: this.tier,
      ownerName: this.ownerName || 'Admin Lead',
      ownerEmail: this.ownerEmail || 'admin@akaaltech.corp',
      residencyRegion: this.residencyRegion,
      storageQuotaGb: this.storageQuotaGb,
      description: this.description
    });
    this.router.navigate(['/administration/enterprise/workspaces', newWs.id]);
  }
}

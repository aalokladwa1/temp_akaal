/**
 * AKAAL Administration — Create Template Asset
 */
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-start justify-between gap-6 pb-5 border-b border-slate-200">
          <div class="flex flex-col gap-1">
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight font-heading">Create Template Asset</h1>
            <p class="text-sm font-medium text-slate-600">
              Author a new reusable configuration template, mapping blueprint, or quality policy.
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white border border-slate-200 rounded-xl shadow-2xs p-6 flex flex-col gap-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="flex flex-col gap-1.5 md:col-span-2">
            <label class="text-xs font-bold text-slate-700 uppercase tracking-wider font-heading">Asset Name</label>
            <input
              type="text"
              [(ngModel)]="name"
              placeholder="e.g. MySQL to Snowflake CDC Replication Blueprint"
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-bold text-slate-700 uppercase tracking-wider font-heading">Asset Family</label>
            <app-custom-select
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <input
              type="text"
              [(ngModel)]="category"
              placeholder="e.g. Database Ingestion"
              class="h-9 px-3 text-xs rounded border border-slate-200 bg-slate-50/50 text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white transition-colors"
            />
          </div>
          ...
          <div class="flex flex-col gap-1.5 md:col-span-2">
            ...
            <textarea
              [(ngModel)]="description"
              rows="2"
              placeholder="Describe the problem this template solves and when engineers should use it..."
          ...
          <div class="flex flex-col gap-1.5 md:col-span-2">
            <label class="text-xs font-bold text-slate-700 uppercase tracking-wider font-heading">Specification Payload (JSON / YAML)</label>
            <textarea
              [(ngModel)]="specPayload"
              rows="8"
...
export class TemplateCreateComponent {
  ...
  public category = 'Database Migration';
  public description = '';
  public specPayload = '{\n  "version": "1.0.0",\n  "dialect": "STANDARD",\n  "parameters": {}\n}';

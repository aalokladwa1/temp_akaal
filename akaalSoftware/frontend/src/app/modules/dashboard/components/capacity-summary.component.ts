    ...
    <div class="p-7 rounded-2xl bg-white border border-slate-200 flex flex-col justify-between gap-6 shadow-xs h-full flex-1">
      ...
      <div class="flex items-center justify-between pb-4 border-b border-slate-200">
        <div class="flex items-center gap-2.5">
          <app-lucide-icon name="cpu" [size]="20" class="text-slate-700 dark:text-slate-300"></app-lucide-icon>
          <h2 class="text-base font-bold text-slate-900 font-heading">Capacity</h2>
        </div>
      </div>

      <!-- Resource Gauges / Empty State -->
      @if (metrics.length === 0) {
        <div class="py-8 flex flex-col items-center justify-center text-center gap-2 my-auto">
          ...
          <span class="text-xs font-bold text-slate-800">Resource metrics unavailable</span>
          <p class="text-[11px] text-slate-500 font-medium max-w-xs">Capacity gauges will update when system metrics are collected.</p>
        </div>
      } @else {
          ...
          @for (cap of metrics; track cap.resource) {
            <div class="py-2.5 first:pt-0 last:pb-0 flex flex-col gap-1.5">
              <div class="flex items-baseline justify-between text-xs">
                ...
                <span class="text-slate-900 font-semibold tabular-nums text-right">
              ...
              <div class="flex justify-between items-baseline text-[11px]">
                ...
                <span class="font-bold text-slate-800 tabular-nums text-right">

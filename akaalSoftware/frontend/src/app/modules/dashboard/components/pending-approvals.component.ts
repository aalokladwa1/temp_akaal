    ...
    <div class="p-7 rounded-2xl bg-white border border-slate-200 flex flex-col justify-between gap-6 shadow-xs h-full flex-1">
      ...
      <div class="flex items-center justify-between pb-4 border-b border-slate-200">
        <div class="flex items-center gap-2.5">
          <app-lucide-icon name="shield-check" [size]="20" class="text-slate-700 dark:text-slate-300"></app-lucide-icon>
      ...
      @if (approvals.length === 0) {
        <div class="py-8 flex flex-col items-center justify-center text-center gap-2 my-auto">
          ...
          <p class="text-[11px] text-slate-500 font-medium max-w-xs">No pending execution barriers require review.</p>
        </div>
      } @else {
...
export class PendingApprovalsComponent {
  ...
  public goToReview(app: PendingApproval): void {
    if (app.migrationId) {
      this.router.navigate(['/cockpit', app.migrationId]);
    } else {

import { Component, Input } from '@angular/core';
import { CommonModule } from '@angular/common';
import { Router } from '@angular/router';
import { ActiveMigration } from '../../../core/models/dashboard.models';
import { LucideIconComponent } from '../../../shared/components/lucide-icon.component';

@Component({
  selector: 'app-active-migrations',
  standalone: true,
  imports: [CommonModule, LucideIconComponent],
  template: `
    <div class="p-7 rounded-2xl bg-white border border-slate-200/90 flex flex-col gap-6 shadow-xs">
      
      <!-- Card Header -->
      <div class="flex items-center justify-between pb-4 border-b border-slate-100">
        <div class="flex items-center gap-3">
          <app-lucide-icon name="arrow-left-right" [size]="20" class="text-blue-600"></app-lucide-icon>
          <h2 class="text-base font-bold text-slate-900 font-heading">Active Migrations</h2>
        </div>
        <button
          type="button"
          (click)="goToMigration()"
          class="text-xs font-bold text-blue-600 hover:text-blue-700 flex items-center gap-1 cursor-pointer">
          <span>View all</span>
          <app-lucide-icon name="chevron-right" [size]="14"></app-lucide-icon>
        </button>
      </div>

      <!-- Migrations Table / List -->
      @if (migrations.length === 0) {
        <div class="py-14 flex flex-col items-center justify-center text-center gap-3">
          <div class="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-500">
            <app-lucide-icon name="database" [size]="24"></app-lucide-icon>
          </div>
          <span class="text-sm font-bold text-slate-800">No active migrations</span>
          <p class="text-xs text-slate-600 font-medium max-w-sm">Running migrations will appear here when the engine reports them.</p>
          <button
            type="button"
            (click)="goToMigration()"
            class="mt-2 px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold shadow-xs transition-colors cursor-pointer flex items-center gap-2">
            <app-lucide-icon name="plus" [size]="14"></app-lucide-icon>
            <span>Go to Migration</span>
          </button>
        </div>
      } @else {
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="border-b border-slate-100 text-xs font-bold text-slate-500 uppercase tracking-wider">
                <th class="pb-3 pr-4">Migration</th>
                <th class="pb-3 px-4">Mode</th>
                <th class="pb-3 px-4">State</th>
                <th class="pb-3 px-4">Operational Status</th>
                <th class="pb-3 pl-4 text-right">Action</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-100 text-xs font-medium">
              @for (m of migrations; track m.id) {
                <tr class="h-14 hover:bg-slate-50/80 transition-colors">
                  <td class="pr-4 py-3">
                    <div class="flex flex-col gap-0.5">
                      <span class="font-bold text-slate-900 hover:text-blue-600 cursor-pointer" (click)="goToMigration()">{{ m.name }}</span>
                      <span class="text-[11px] text-slate-600">{{ m.sourceEngine }} &rarr; {{ m.targetEngine }}</span>
                    </div>
                  </td>
                  <td class="px-4 py-3">
                    <span class="px-2.5 py-1 rounded-md text-[11px] font-bold bg-blue-50 text-blue-700 border border-blue-200/60">
                      {{ formatMode(m.mode) }}
                    </span>
                  </td>
                  <td class="px-4 py-3">
                    <span 
                      class="px-2.5 py-1 rounded-full text-[11px] font-bold"
                      [class.bg-emerald-50]="m.state === 'RUNNING'"
                      [class.text-emerald-700]="m.state === 'RUNNING'"
                      [class.border]="m.state === 'RUNNING'"
                      [class.border-emerald-200]="m.state === 'RUNNING'"
                      [class.bg-amber-50]="m.state !== 'RUNNING'"
                      [class.text-amber-700]="m.state !== 'RUNNING'">
                      {{ m.state }}
                    </span>
                  </td>
                  <td class="px-4 py-3">
                    @if (m.mode === 'M1_BULK' || m.mode === 'M2_BULK_CDC') {
                      <div class="flex flex-col gap-1 w-48">
                        <div class="flex justify-between text-[11px] text-slate-600 font-medium">
                          <span>Progress: {{ m.progressPercent ?? '—' }}%</span>
                          <span>{{ formatNumber(m.throughputRowsSec) }} r/s</span>
                        </div>
                        <div class="w-full h-1.5 rounded-full bg-slate-100 overflow-hidden">
                          <div class="h-full bg-blue-600 rounded-full" [style.width.%]="m.progressPercent ?? 0"></div>
                        </div>
                      </div>
                    }
                    @if (m.mode === 'M3_CDC_CONTINUOUS') {
                      <div class="flex items-center gap-3 text-[11px]">
                        <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                        <span class="text-emerald-700 font-bold">Lag: {{ m.cdcLagMs ?? '—' }}ms</span>
                        <span class="text-slate-400">&bull;</span>
                        <span class="text-slate-700 font-semibold">{{ formatNumber(m.cdcBacklogEvents) }} evts</span>
                      </div>
                    }
                  </td>
                  <td class="pl-4 py-3 text-right">
                    <button
                      type="button"
                      (click)="goToMigration()"
                      class="px-3.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs shadow-2xs transition-colors cursor-pointer">
                      Open
                    </button>
                  </td>
                </tr>
              }
            </tbody>
          </table>
        </div>
      }

    </div>
  `
})
export class ActiveMigrationsComponent {
  @Input() public migrations: ActiveMigration[] = [];

  constructor(private router: Router) {}

  public goToMigration(): void {
    this.router.navigate(['/migration']);
  }

  public formatMode(mode: string): string {
    switch (mode) {
      case 'M1_BULK': return 'M1 • Bulk';
      case 'M2_BULK_CDC': return 'M2 • Bulk + CDC';
      case 'M3_CDC_CONTINUOUS': return 'M3 • Live CDC';
      case 'M4_INCREMENTAL': return 'M4 • Incremental';
      case 'M5_STATE_SYNC': return 'M5 • State Sync';
      case 'M6_SCHEMA_ONLY': return 'M6 • Schema Only';
      case 'M7_DATA_ONLY': return 'M7 • Data Only';
      case 'M8_VALIDATION_ONLY': return 'M8 • Validation';
      default: return mode;
    }
  }

  public formatNumber(num?: number | null): string {
    if (num === undefined || num === null) return '—';
    if (num >= 1_000_000_000) return (num / 1_000_000_000).toFixed(2) + 'B';
    if (num >= 1_000_000) return (num / 1_000_000).toFixed(1) + 'M';
    if (num >= 1_000) return (num / 1_000).toFixed(0) + 'k';
    return num.toLocaleString();
  }
}

"""
AKAAL Medium-Scale Migration — Step 3: Execution, Interruption & Resume Test, Data Validation, CDC & Reporting.

Executes:
1. Creation of 50 Target Tables on MySQL in strict dependency order (Level 1 -> 5).
2. Streaming Data Migration of 131,115 rows in batches of 2,000 with checkpointing.
...
MYSQL_ROOT = dict(host='127.0.0.1', port=3306, user='root', password='', charset='utf8mb4')
...
async def main():
    print("=================================================================")
    print("      AKAAL MEDIUM-SCALE MIGRATION EXECUTION & VALIDATION")
    print("=================================================================\n")
    ...
    audit.log(
        ...
        actor="akaal-medium-runner",
        description="Starting medium-scale migration execution (50 tables, 131k rows)",
        project_id="medium-mig-001",
        details={"source": "postgresql/public", "target": "mysql/akaal_medium_tgt"}
    )

    # 1. INITIALIZE TARGET DATABASE
    print("[STAGE 1/7] Initializing Target MySQL Database (akaal_medium_tgt)...")
    my_root = pymysql.connect(**MYSQL_ROOT)
    with my_root.cursor() as cur:
        cur.execute("DROP DATABASE IF EXISTS akaal_medium_tgt;")
        cur.execute("CREATE DATABASE akaal_medium_tgt CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;")
    my_root.close()

    # Create 50 MySQL Target Tables matching PostgreSQL source schema
    ...
    # Helper function to generate MySQL DDL for each table
    for t_name in table_names:
        ...
        col_defs = []
        for col_name, data_type, max_len, is_null, col_def in cols:
            dt = data_type.upper()
            ...
            # Single AUTO_INCREMENT PK if col in pk_cols and pk_cols is length 1
            if len(pk_cols) == 1 and col_name == pk_cols[0] and dt in ('INTEGER', 'INT', 'BIGINT'):
                col_defs.append(f"`{col_name}` {m_type} AUTO_INCREMENT PRIMARY KEY")
            else:
    ...
    my_conn.commit()
    print(f"  [OK] 50 Target tables created on MySQL (akaal_medium_tgt).")

    # 2. DATA MIGRATION WITH STREAMING & CHECKPOINTING
    print("\n[STAGE 2/7] Streaming Data Migration of 131,115 rows with Checkpointing...")
    
    ckpt_dir = os.path.join(os.getcwd(), "artifacts", "medium_checkpoints")
    ...
    for t_name in table_names:
        cursor_name = f"stream_{t_name}"
        pg_dict_cur = pg_conn.cursor(name=cursor_name, cursor_factory=psycopg2.extras.RealDictCursor)
        pg_dict_cur.itersize = batch_size
        pg_dict_cur.execute(f"SELECT * FROM public.{t_name};")
        ...
        while True:
            ...
            ckpt = CheckpointRecord(
                checkpoint_id=f"ckpt-{t_name}-batch-{b_num}",
                project_id="medium-mig-001",
                migration_id="mig-med-001",
            ...
            if total_migrated >= 50000 and not simulated_interruption_done:
                print(f"\n  [⚡ INTERRUPTION TEST] Simulating Process Crash at total_migrated={total_migrated:,} rows (Checkpoint: {ckpt.checkpoint_id})...")
                print("  [⚡ INTERRUPTION TEST] Restarting AKAAL Migration Engine & loading latest checkpoint state...")
                
                restored_ckpt = await ckpt_storage.read_latest("medium-mig-001", "mig-med-001", t_name)
    ...
    print(f"  [OK] Data migration completed: {total_migrated:,} rows migrated across 50 tables with {checkpoint_count} checkpoints.")
    ...
    print(f"  [OK] Source Total Rows: {src_total_rows:,} | Target Total Rows: {tgt_total_rows:,}")
    ...
    for t_name in sample_check_tables:
        ...
        v_cur = pg_conn.cursor(name=f"chk_{t_name}", cursor_factory=psycopg2.extras.RealDictCursor)
        v_cur.itersize = 2000
        v_cur.execute(f"SELECT * FROM public.{t_name}{order_clause};")
        while True:
            chunk = v_cur.fetchmany(2000)
            if not chunk:
                break
            for r in chunk:
                row_str = json.dumps({k: norm(v) for k, v in sorted(r.items())}, sort_keys=True)
    ...
    cdc_session = await cdc.start_cdc_session(source_engine='postgresql', source_db='postgres', target_dbs=['akaal_medium_tgt'])
    ...
    event_ins = CDCEvent(
        ...
        after_state={'customer_id': 99999, 'first_name': 'CDC_Test', 'last_name': 'User', 'email': 'cdc.test@akaal.com'}
    )
    ...
    req_pre = ReportRequestDTO(report_type="PRE_MIGRATION", migration_id="mig-med-001", export_format="JSON")
    rep_pre = await facade.generate_report(req_pre)

    req_exec = ReportRequestDTO(report_type="EXECUTIVE_SUMMARY", migration_id="mig-med-001", export_format="JSON")
    rep_exec = await facade.generate_report(req_exec)

    pkg = await facade.generate_audit_package(migration_id="mig-med-001", report_types=["PRE_MIGRATION", "EXECUTIVE_SUMMARY"])
    ...
    audit.log(
        ...
        actor="akaal-medium-runner",
        description="Medium-scale migration completed successfully",
        project_id="medium-mig-001",
    ...
    print("\n=================================================================")
    print("                MEDIUM-SCALE MIGRATION SUMMARY")
    print("=================================================================")
    ...
    with open("artifacts/medium_migration_results.json", "w", encoding="utf-8") as f:
    ...
    if len(validation_issues) > 0:
        print("❌ MEDIUM-SCALE MIGRATION FAILED")
        sys.exit(1)
    else:
        print("✅ MEDIUM-SCALE MIGRATION PASSED")
        sys.exit(0)

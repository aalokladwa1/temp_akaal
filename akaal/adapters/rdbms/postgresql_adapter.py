...
class PostgreSQLAdapter(BaseAdapter):
    ...
    async def read_lob_chunk(
        ...
        if self.mock_mode:
            # Emulates large text blob chunk reading
    ...
    async def write_lob_chunk(
        ...
        if self.mock_mode:
            logger.info("[PostgreSQLAdapter] Mock LOB write chunk table=%s col=%s offset=%d len=%d",
                        table_name, lob_column, offset, len(chunk_data))
            return
            ...
            for idx, c in enumerate(cols, 1):
                ...
                col_models.append(
                    CanonicalColumn(
                        ...
                        source_native_type=c.get("type", "TEXT"),
                        canonical_type="TEXT",

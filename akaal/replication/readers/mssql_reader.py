"""
AKAAL Replication Engine — Canonical MSSQL Physical Reader Module
==================================================================
High-performance Microsoft SQL Server physical partition reader using pyodbc
...
logger = logging.getLogger("akaal.replication.readers.mssql_reader")


class MSSQLPhysicalReader(IPhysicalReader):
    """
    Canonical High-performance MSSQL physical partition reader using pyodbc
            ...
            try:
                ...
                if attempt == 4:
                    if self.params.get("allow_mock_fallback", False):
                        from unittest.mock import MagicMock
                        logger.warning(f"[MSSQLPhysicalReader] Test mock fallback triggered for {host}:{port}/{dbname}: {e}")
                        self.conn = MagicMock()
                        self.cursor = self.conn.cursor.return_value
                        return
                    raise e
    ...
    def open_partition(self, partition: TransportPartition, last_committed_key: Optional[Any] = None) -> None:
        ...
        schema_name = partition.schema_name or "dbo"
        table_name = partition.table_name
        quoted_table = f"[{schema_name}].[{table_name}]"

        # Fetch column metadata
        self.cursor = self.conn.cursor()
        self.cursor.execute(f"SELECT TOP 0 * FROM {quoted_table}")
    ...
    def read_batch(self, batch_size: int) -> Tuple[List[Tuple], BatchMetadata]:
        if hasattr(self.conn, "_mock_name") or type(self.conn).__name__ == "MagicMock":
            if not self.params.get("allow_test_mock_harness", False):
                raise RuntimeError("MSSQLPhysicalReader requires a valid physical database connection cursor. Mock fallback is disallowed in physical production readers.")
            ...
            mock_data = [(1, "mssql_mock_1", 1000.0), (2, "mssql_mock_2", 2000.0)]

"""
AKAAL Replication Engine — Canonical Range Partitioner Module
==============================================================
Generates TransportPartition objects for inter-table and intra-table Primary Key range partitioning.
"""
...
logger = logging.getLogger("akaal.replication.partitioning.range_partitioner")


class RangePartitioner:
    """Canonical range partitioner supporting numeric and date range chunking."""
    ...
    def generate_partitions_for_table(
        ...
        if (
            ...
            logger.info(f"[RANGE PARTITIONER] Created {len(partitions)} intra-table partitions for {table_name}")
        else:

...
class SQLServerRoutineRenderer(BaseRoutineRenderer):
    def render(self) -> str:
        params_list = []
        ...
        if self.aoir.kind == RoutineKind.FUNCTION:
            ret_type = self.aoir.return_spec.data_type if self.aoir.return_spec else "VARCHAR(MAX)"
            sql = f"CREATE FUNCTION {self.aoir.name}({params_decl}) RETURNS {ret_type} AS\nBEGIN\n"
        else:
            sql = f"CREATE PROCEDURE {self.aoir.name} {params_decl} AS\nBEGIN\n"

...
class ProcedureTranslator(BaseObjectTranslator):
    """Placeholder translator for Procedure database objects."""
    ...
    def translate_drop(self, obj, context: Dict[str, Any], quoter, capabilities, builder) -> TranslationResult:
        ...
        sql = f"DROP PROCEDURE {full_proc_name}"
        rollback_sql = f"CREATE PROCEDURE {full_proc_name}"
        return TranslationResult(sql=sql, rollback_sql=rollback_sql)
...
ObjectTranslatorRegistry.register(ObjectType.PROCEDURE, ProcedureTranslator())

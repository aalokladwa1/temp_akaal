...
class TestSqlserverToOracle(unittest.IsolatedAsyncioTestCase):

    async def asyncSetUp(self):
        try:
            self.conn_src = get_connection("sqlserver")
            self.conn_tgt = get_connection("oracle_tgt")
    ...
    async def test_sqlserver_to_oracle_migration(self):
        # 1. Reset both databases
        reset_source_database("sqlserver")
        reset_target_database("oracle_tgt")
        
        # 2. Apply schema and seed to source
        apply_schema("sqlserver")
        apply_seed_data("sqlserver")
        
        # 3. Apply schema to target
        apply_schema("oracle_tgt")
        ...
        src_conn.username = SQLSERVER_CONFIG["user"]
        src_conn.password = SQLSERVER_CONFIG["password"]
        src_conn.trusted_connection = SQLSERVER_CONFIG["trusted_connection"]
        
        tgt_conn = ConnectionConfig(
            system_type=SystemType.ORACLE,
            host=ORACLE_TARGET_CONFIG["host"],
            port=ORACLE_TARGET_CONFIG["port"],
            database_name=ORACLE_TARGET_CONFIG["database"],
            credentials_ref="oracle_tgt_creds",
            read_only=False
        )
        tgt_conn.username = ORACLE_TARGET_CONFIG["user"]
        tgt_conn.password = ORACLE_TARGET_CONFIG["password"]
        ...
        config = MigrationConfig(
            ...
            project_name="SQL Server to Oracle Validation",
            auto_approve=True,
        ...
        validate_table_counts(self.conn_tgt, "oracle_tgt", 5)
        ...
        for table in tables:
            validate_row_counts(self.conn_tgt, "oracle_tgt", table, expected_counts[table])
            validate_constraints(self.conn_src, self.conn_tgt, ("sqlserver", "oracle_tgt"), table)
            validate_indexes(self.conn_src, self.conn_tgt, ("sqlserver", "oracle_tgt"), table)
            validate_foreign_keys(self.conn_tgt, "oracle_tgt", table)
            validate_data_integrity(self.conn_src, self.conn_tgt, ("sqlserver", "oracle_tgt"), table)
            
        validate_json_columns(self.conn_src, self.conn_tgt, ("sqlserver", "oracle_tgt"), "products", "attributes")
        validate_json_columns(self.conn_src, self.conn_tgt, ("sqlserver", "oracle_tgt"), "audit_logs", "new_value")
        validate_blob_columns(self.conn_src, self.conn_tgt, ("sqlserver", "oracle_tgt"), "audit_logs", "raw_payload")
        ...
        print("\n" + "="*50)
        print("MIGRATION SMOKE TEST RESULT: SQL Server -> Oracle")
        print("="*50)

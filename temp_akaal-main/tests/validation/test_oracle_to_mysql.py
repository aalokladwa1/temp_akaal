...
class TestOracleToMysql(unittest.IsolatedAsyncioTestCase):

    async def asyncSetUp(self):
        try:
            self.conn_src = get_connection("oracle_src")
            self.conn_tgt = get_connection("mysql")
    ...
    async def test_oracle_to_mysql_migration(self):
        # 1. Reset both databases
        reset_source_database("oracle_src")
        reset_target_database("mysql")
        
        # 2. Apply schema and seed to source
        apply_schema("oracle_src")
        apply_seed_data("oracle_src")
        ...
        src_conn = ConnectionConfig(
            ...
            host=ORACLE_SOURCE_CONFIG["host"],
            port=ORACLE_SOURCE_CONFIG["port"],
            database_name=ORACLE_SOURCE_CONFIG["database"],
            credentials_ref="oracle_src_creds",
            read_only=True
        )
        src_conn.username = ORACLE_SOURCE_CONFIG["user"]
        src_conn.password = ORACLE_SOURCE_CONFIG["password"]
        ...
        tgt_conn.username = MYSQL_CONFIG["user"]
        tgt_conn.password = MYSQL_CONFIG["password"]
        ...
        config = MigrationConfig(
            ...
            project_name="Oracle to MySQL Validation",
            auto_approve=True,
        ...
        validate_table_counts(self.conn_tgt, "mysql", 5)
        ...
        for table in tables:
            validate_row_counts(self.conn_tgt, "mysql", table, expected_counts[table])
            validate_constraints(self.conn_src, self.conn_tgt, ("oracle_src", "mysql"), table)
            validate_indexes(self.conn_src, self.conn_tgt, ("oracle_src", "mysql"), table)
            validate_foreign_keys(self.conn_tgt, "mysql", table)
            validate_data_integrity(self.conn_src, self.conn_tgt, ("oracle_src", "mysql"), table)
            
        validate_json_columns(self.conn_src, self.conn_tgt, ("oracle_src", "mysql"), "products", "attributes")
        validate_json_columns(self.conn_src, self.conn_tgt, ("oracle_src", "mysql"), "audit_logs", "new_value")
        validate_blob_columns(self.conn_src, self.conn_tgt, ("oracle_src", "mysql"), "audit_logs", "raw_payload")
        ...
        print("\n" + "="*50)
        print("MIGRATION SMOKE TEST RESULT: Oracle -> MySQL")
        print("="*50)

...
class TestOracleToPostgres(unittest.IsolatedAsyncioTestCase):

    async def asyncSetUp(self):
        try:
            self.conn_src = get_connection("oracle_src")
            self.conn_tgt = get_connection("postgres")
    ...
    async def test_oracle_to_postgres_migration(self):
        # 1. Reset both databases
        reset_source_database("oracle_src")
        reset_target_database("postgres")
        ...
        config = MigrationConfig(
            ...
            project_name="Oracle to PostgreSQL Validation",
            auto_approve=True,
        ...
        validate_table_counts(self.conn_tgt, "postgres", 5)
        ...
        for table in tables:
            validate_row_counts(self.conn_tgt, "postgres", table, expected_counts[table])
            validate_constraints(self.conn_src, self.conn_tgt, ("oracle_src", "postgres"), table)
            validate_indexes(self.conn_src, self.conn_tgt, ("oracle_src", "postgres"), table)
            validate_foreign_keys(self.conn_tgt, "postgres", table)
            validate_data_integrity(self.conn_src, self.conn_tgt, ("oracle_src", "postgres"), table)
            
        validate_json_columns(self.conn_src, self.conn_tgt, ("oracle_src", "postgres"), "products", "attributes")
        validate_json_columns(self.conn_src, self.conn_tgt, ("oracle_src", "postgres"), "audit_logs", "new_value")
        validate_blob_columns(self.conn_src, self.conn_tgt, ("oracle_src", "postgres"), "audit_logs", "raw_payload")
        ...
        print("\n" + "="*50)
        print("MIGRATION SMOKE TEST RESULT: Oracle -> PostgreSQL")
        print("="*50)

...
const MIGRATION_STATUS_META: Record<MigrationStatus, { label: string; color: string; bg: string; border: string; animated: boolean }> = {
  running:      { label: 'Running',      color: '#38BDF8', bg: 'rgba(56,189,248,0.08)',  border: 'rgba(56,189,248,0.2)',  animated: true },
  paused:       { label: 'Paused',       color: '#F59E0B', bg: 'rgba(245,158,11,0.08)',  border: 'rgba(245,158,11,0.2)',  animated: false },
  failed:       { label: 'Failed',       color: '#EF4444', bg: 'rgba(239,68,68,0.08)',   border: 'rgba(239,68,68,0.2)',   animated: false },
  completed:    { label: 'Completed',    color: '#22C55E', bg: 'rgba(34,197,94,0.08)',   border: 'rgba(34,197,94,0.2)',   animated: false },
  queued:       { label: 'Queued',       color: '#94A3B8', bg: 'rgba(148,163,184,0.08)', border: 'rgba(148,163,184,0.2)', animated: false },
  retrying:     { label: 'Retrying',     color: '#F59E0B', bg: 'rgba(245,158,11,0.08)',  border: 'rgba(245,158,11,0.2)',  animated: true },
  rolling_back: { label: 'Rolling Back', color: '#F59E0B', bg: 'rgba(245,158,11,0.08)',  border: 'rgba(245,158,11,0.2)',  animated: true },
};
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <Card>
                  ...
                  <div ref={logContainerRef} className="overflow-y-auto font-mono" style={{ maxHeight: '260px', background: '#0B1220' }} aria-live="polite" aria-label="Log output">
                    {loading ? (
                      ...
                      <div>
                        {filteredLogs.map(log => (
                          <div key={log.id} className="flex items-start gap-2 px-3 py-1.5 transition-colors" style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}
                            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.02)'; }}
                            onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'transparent'; }}
                          >
                            <span style={{ color: '#374151', fontFamily: "'JetBrains Mono',monospace", fontSize: '10px', flexShrink: 0, marginTop: '1px' }}>{log.timestamp}</span>
                            ...
                            {log.source && <span style={{ color: '#374151', fontFamily: "'JetBrains Mono',monospace", fontSize: '10px', flexShrink: 0 }}>[{log.source}]</span>}
                            <span style={{ color: log.level === 'ERROR' ? '#FCA5A5' : log.level === 'WARN' ? '#FDE68A' : log.level === 'SUCCESS' ? '#86EFAC' : '#94A3B8', fontFamily: "'JetBrains Mono',monospace", fontSize: '10px', lineHeight: '1.5', wordBreak: 'break-all' }}>{log.message}</span>
                          </div>
                        ))}
                      </div>
                    )}
                        ...
                        <thead>
                          <tr style={{ background: '#111827' }}>
                            {['Migration', 'Timestamp', 'Rows At', 'Size', 'Interval', 'Restore', 'Last Test'].map(h => (
                              <th key={h} className="px-3 py-2 text-left" style={{ color: '#64748B', fontFamily: "'Inter',sans-serif", fontSize: '10px', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', borderBottom: '1px solid #2A3647', whiteSpace: 'nowrap' }}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>

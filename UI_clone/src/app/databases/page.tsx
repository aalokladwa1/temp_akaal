      ...
      <div className="relative">
        ...
        {notifOpen && (
          <div className="absolute right-0 top-full mt-1 rounded-lg overflow-hidden z-50" style={{ width: '300px', background: 'var(--akaal-surface)', border: '1px solid var(--akaal-border)', boxShadow: '0 8px 32px var(--akaal-shadow)' }} role="dialog" aria-label="Notifications">
            ...
            {[
              { title: 'Connection Lost', detail: 'prod-mssql-crm is unreachable', time: '2m ago', color: 'var(--akaal-error)', href: '/databases' },
              { title: 'Certificate Expiry', detail: 'prod-oracle-legacy cert expires in 52 days', time: '1h ago', color: 'var(--akaal-warning)', href: '/databases' },
              { title: 'Health Degraded', detail: 'dev-mongodb-events latency spike: 89ms', time: '3h ago', color: 'var(--akaal-warning)', href: '/live-monitor' },
            ].map((n, i) => (
...
function LeftPanel({
  ...
  return (
    <aside className="flex flex-col flex-shrink-0 overflow-y-auto" style={{ width: '200px', background: 'var(--akaal-sidebar-bg)', borderRight: '1px solid var(--akaal-card-border)' }} aria-label="Database categories">
      <div className="px-3 py-3 flex-shrink-0" style={{ borderBottom: '1px solid var(--akaal-card-border)' }}>
        <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--akaal-text-muted)', fontFamily: "'JetBrains Mono', monospace", fontSize: '9px', letterSpacing: '0.12em' }}>Categories</p>
      </div>
        ...
        {categories.map(cat => {
          ...
          return (
            ...
            >
              ...
              <span className="text-xs tabular-nums flex-shrink-0" style={{ color: 'var(--akaal-text-muted)', fontFamily: "'JetBrains Mono', monospace", fontSize: '10px' }}>{cat.count}</span>
            </button>
          );
        })}
      </nav>

      <div className="px-3 py-3 flex-shrink-0" style={{ borderTop: '1px solid var(--akaal-card-border)', borderBottom: '1px solid var(--akaal-card-border)', marginTop: '4px' }}>
        <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: 'var(--akaal-text-muted)', fontFamily: "'JetBrains Mono', monospace", fontSize: '9px', letterSpacing: '0.12em' }}>Filters</p>
      </div>
...
function ConnectionTable({
  connections, loading, onSelect, selectedId, onToggleFavorite, onTestConn, onDeleteConn,
}: {
  connections: DBConnection[];
  loading: boolean;
  ...
  onTestConn?: (conn: DBConnection) => void;
        ...
        {loading ? (
          ...
          <div className="p-8">
            <EnterpriseEmptyState
              title={search ? `No connections matching "${search}"` : 'No database connections configured'}
              description={search ? 'No enterprise database connections matched your search query. Try clearing search keywords or resetting filters.' : 'No database connections have been connected to this environment yet. Connect a database engine to manage schemas and migrations.'}
              primaryAction={search ? { label: 'Clear Search', onClick: () => setSearch('') } : { label: 'Connect Database', href: '/databases/connect' }}
              secondaryAction={search ? { label: 'Connect Database', href: '/databases/connect' } : undefined}
            />
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td style={tdStyle} onClick={e => e.stopPropagation()}>
                      <div className="flex items-center gap-1">
                        <button type="button" onClick={() => onTestConn?.(conn)}
                          className="px-2 py-1 rounded text-xs transition-all duration-150 focus:outline-none focus-visible:ring-1"
                          style={{ color: '#38BDF8', background: 'transparent', fontFamily: "'Inter', sans-serif" }}
                          aria-label={`Test ${conn.name}`}
                          onMouseEnter={e => { e.currentTarget.style.background = 'rgba(56,189,248,0.1)'; }}
                          onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
                        >Test</button>
                        <button type="button" onClick={() => onSelect(conn)}
                          className="px-2 py-1 rounded text-xs transition-all duration-150 focus:outline-none focus-visible:ring-1"
                          style={{ color: '#64748B', background: 'transparent', fontFamily: "'Inter', sans-serif" }}
                          aria-label={`Edit ${conn.name}`}
                          onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; e.currentTarget.style.color = '#CBD5E1'; }}
                          onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#64748B'; }}
                        >Inspect</button>
                        <button type="button" onClick={() => onDeleteConn?.(conn.id)}
                          className="px-2 py-1 rounded text-xs transition-all duration-150 focus:outline-none focus-visible:ring-1"
                          style={{ color: '#EF4444', background: 'transparent', fontFamily: "'Inter', sans-serif" }}
...
export default function DatabasesPage() {
  ...
  const handleTestConn = useCallback((conn: DBConnection) => {
    setConnections(cs => cs.map(c => c.id === conn.id ? { ...c, lastChecked: 'Just now', latencyMs: Math.floor(Math.random() * 15) + 5 } : c));
  }, []);

  const handleDeleteConn = useCallback((id: string) => {
    setConnections(cs => cs.filter(c => c.id !== id));
    setSelectedConn(prev => prev?.id === id ? null : prev);
  }, []);

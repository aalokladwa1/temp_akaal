"""
akaalEngine.schema.ddl.providers.mssql
======================================
Target DDL emitter for Microsoft SQL Server (T-SQL).
"""
...
class MSSQLDDLEmitter(BaseTargetDDLEmitter):
    ...
    def emit_udt_artifacts(self, udt: CanonicalUDT) -> List[StructuredDDLArtifact]:
        ...
        if udt.underlying_type:
            ...
            sql = f"CREATE TYPE {qual} FROM {udt.underlying_type};"
            return [
                StructuredDDLArtifact(
                    object_type="UDT",
    ...
    def emit_column_definition(self, col: CanonicalColumn, source_engine: str = "GENERIC") -> Tuple[str, ConversionSafety, List[str]]:
        warnings = []
        emission = CanonicalTypeRegistry.emit_target_type("MSSQL", col.canonical_type)
        ...
        if not col.nullable and not col.is_identity:
            col_sql += " NOT NULL"
        elif col.nullable and not col.is_identity:
            col_sql += " NULL"
    ...
    def emit_table_artifacts(self, table: CanonicalTable, source_engine: str = "GENERIC") -> List[StructuredDDLArtifact]:
        ...
        s_name = table.schema_name or "dbo"
        ...
        for idx in table.indexes:
            if idx.columns:
                ...
                clustered_str = "CLUSTERED " if idx.is_clustered else "NONCLUSTERED "
                ...
                idx_sql = f"CREATE {unique_str}{clustered_str}INDEX {q_idx} ON {qual_tbl} ({cols_str}){include_str}{where_str};"
                artifacts.append(
                    StructuredDDLArtifact(
                        object_type="INDEX",
    ...
    def emit_view_artifacts(self, view: CanonicalView, source_engine: str = "GENERIC") -> List[StructuredDDLArtifact]:
        ...
        sql = f"CREATE OR ALTER VIEW {qual} AS\n{view.definition_sql};"
        return [
            StructuredDDLArtifact(
                object_type="VIEW",

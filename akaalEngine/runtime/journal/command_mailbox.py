"""
akaalEngine.runtime.journal.command_mailbox
============================================
Durable Command Mailbox for storing START, PAUSE, RESUME, CHECKPOINT, STOP, ROLLBACK, CANCEL commands
with acknowledgement and replay support. Mined from `akaal/runtime/journal/command_mailbox.py`.
"""
...
logger = logging.getLogger("akaalEngine.runtime.journal")
...
class DurableCommandMailbox:
    ...
    def __init__(self, migration_id: str, journal_dir: Optional[str] = None) -> None:
        ...
        base_dir = journal_dir or os.path.join(os.path.expanduser("~"), ".akaalEngine", "journal")
    ...
    def post_command(self, command_type: str, parameters: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        logger.info(f"[CommandMailbox] Posted command '{command_type}' (ID: {cmd_id}) for migration '{self.migration_id}'.")
        return entry

"""
akaalEngine.transport.drivers.sap_ase
======================================
Canonical SAP ASE (Sybase Adaptive Server Enterprise) physical Transport driver
(P7A Campaign B, provider #42).

Unlike PostgreSQL/MySQL-wire-compatible providers, ASE does not enable ANSI
double-quoted identifiers by default (`SET QUOTED_IDENTIFIER` is session-scoped and
not assumed here) -- reusing `GenericSQLSourceReader`/`GenericSQLTargetWriter`
unmodified would silently emit invalid SQL against a real ASE server. This driver is
therefore a standalone DB-API 2.0 reader/writer (not a GenericSQL subclass) using
unquoted identifiers, connected via `pytds` (FreeTDS/TDS protocol, paramstyle
'qmark') or `pyodbc` (paramstyle 'qmark').

No native `bcp`/replication-server CDC claim is made -- no capture module exists here.
"""
...
logger = logging.getLogger("akaalEngine.transport.drivers.sap_ase")

SAP_ASE_DEFAULT_PORT = 5000


class SAPASESourceReader(SourceReader):
    """Real SAP ASE SourceReader using standard DB-API 2.0 cursor iteration with
    ...
    def read_batch(self, batch_size: int = 5000) -> Optional[TransportBatch]:
        ...
        meta = TransportBatchMetadata(
            batch_id=f"sap-ase-batch-{self.sequence_number}",
...
class SAPASETargetWriter(TargetWriter):
    """Real SAP ASE TargetWriter using executemany() with unquoted identifiers and a real
    ...
    def write_batch(
        ...
        if not self.cursor:
            if self.params.get("db_connection"):
                ...
                raise TransportWriteError("SAPASETargetWriter has no active database connection or cursor.")
    ...
    def verify_uncertain_commit(
        ...
        try:
            ...
            logger.warning(f"[SAPASETargetWriter] verify_uncertain_commit physical check failed: {exc}")
            return CommitOutcomeState.UNKNOWN_COMMIT_OUTCOME

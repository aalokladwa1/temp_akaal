"""
akaalEngine.connection.providers.storage.gcs
============================================
Canonical Google Cloud Storage (GCS) Provider Strategy.
Supports google-cloud-storage, bucket discovery, and streaming blob upload/download.
"""
...
logger = logging.getLogger("akaalEngine.connection.providers.gcs")


class GCSProviderStrategy(BaseProviderStrategy):
    """Canonical Google Cloud Storage provider strategy."""
    ...
    def get_static_manifest(self) -> StaticCapabilityManifest:
        return StaticCapabilityManifest(
            ...
            capabilities={
                ...
                "RESUMABLE_UPLOAD": CapabilitySupportStatus.SUPPORTED,
            },
    ...
    def is_dependency_available(self) -> Tuple[bool, str]:
        try:
            ...
            return True, "google-cloud-storage available."
        except ImportError:
            return False, "google-cloud-storage not installed. Install via 'pip install google-cloud-storage'."
    ...
    def connect(
        ...
        if not avail:
            raise DependencyMissingError(
                ConnectionFailure(
                    error_code="GCS_DEPENDENCY_MISSING",
        ...
        if sa_info:
            try:
                ...
                return storage.Client(project=project_id, credentials=gcp_creds)
            except Exception as exc:
                raise AuthenticationError(
                    ConnectionFailure(
                        error_code="GCP_SA_CREDENTIAL_CONSTRUCTION_FAILED",
                        category=FailureCategory.AUTHENTICATION_FAILURE,
                        message=f"Failed to construct Google Service Account credentials: {redact_text(str(exc))}",
                        retryable=False,
                        provider_id=self.PROVIDER_ID,
                    )
                ) from exc
        ...
        if spec.auth_spec and spec.auth_spec.service_account_json_ref:
            raise AuthenticationError(
                ConnectionFailure(
                    error_code="GCP_EXPLICIT_CREDENTIALS_MISSING",
                    category=FailureCategory.AUTHENTICATION_FAILURE,
    ...
    def validate(self, connection: Any) -> bool:
        if connection is None:
            return False
        try:
            list(connection.list_buckets(max_results=1))
            return True
        except Exception:
            return False
    ...
    def attest_physical_identity(
        ...
        return PhysicalEndpointIdentity(
            ...
            provider_version=self.PROVIDER_VERSION,
            role=spec.role,
            resolved_host="storage.googleapis.com",
            resolved_ip=resolved_route.resolved_ip,
            resolved_port=443,
            server_version="Google Cloud Storage",
    ...
    def probe_capabilities(
        ...
        return ProbedCapabilitySnapshot(
            provider_id=self.PROVIDER_ID,
            endpoint_fingerprint="gcs-attested",
            capabilities={
                "BUCKET_DISCOVERY": CapabilitySupportStatus.SUPPORTED,
                "OBJECT_READ": CapabilitySupportStatus.SUPPORTED,
                "OBJECT_WRITE": CapabilitySupportStatus.SUPPORTED,
    ...
    def probe_permissions(
        ...
        return PermissionSnapshot(
            provider_id=self.PROVIDER_ID,
            endpoint_fingerprint="gcs-attested",
            granted_privileges=["storage.objects.get", "storage.objects.create"],
            missing_privileges=[],

"""
akaalEngine.connection.providers.nosql.cassandra
================================================
Canonical Apache Cassandra Provider Strategy.
Supports cassandra-driver, token-aware routing, and CQL schema discovery.
"""
...
logger = logging.getLogger("akaalEngine.connection.providers.cassandra")


class CassandraProviderStrategy(BaseProviderStrategy):
    """Canonical Apache Cassandra provider strategy."""
    ...
    def get_static_manifest(self) -> StaticCapabilityManifest:
        return StaticCapabilityManifest(
            ...
            capabilities={
                ...
                "TOKEN_AWARE_ROUTING": CapabilitySupportStatus.SUPPORTED,
                "CQL_BATCH": CapabilitySupportStatus.SUPPORTED,
                "CDC_LOG_CAPTURE": CapabilitySupportStatus.PARTIAL,  # Cassandra commitlog CDC
            },
    ...
    def is_dependency_available(self) -> Tuple[bool, str]:
        try:
            ...
            return True, "cassandra-driver available."
        except ImportError:
            return False, "cassandra-driver not installed. Install via 'pip install cassandra-driver'."
    ...
    def connect(
        ...
        if not avail:
            raise DependencyMissingError(
                ConnectionFailure(
                    error_code="CASSANDRA_DEPENDENCY_MISSING",
        ...
        session = cluster.connect(keyspace=spec.database_name)
        return session
    ...
    def attest_physical_identity(
        ...
        if connection:
            try:
                ...
                if row:
                    ...
                    server_ver = f"Cassandra {row.release_version}"
            except Exception:
                pass
        ...
        return PhysicalEndpointIdentity(
            ...
            principal_identity=spec.auth_spec.username if spec.auth_spec else "cassandra",
            route_type=spec.route_spec.route_type,
            topology_role="DISTRIBUTED_RING",
        )
    ...
    def probe_capabilities(
        ...
        return ProbedCapabilitySnapshot(
            provider_id=self.PROVIDER_ID,
            endpoint_fingerprint="cassandra-attested",
            capabilities={
                "SCHEMA_DISCOVERY": CapabilitySupportStatus.SUPPORTED,
                "BULK_READ": CapabilitySupportStatus.SUPPORTED,
                "BULK_WRITE": CapabilitySupportStatus.SUPPORTED,
    ...
    def probe_permissions(
        ...
        return PermissionSnapshot(
            provider_id=self.PROVIDER_ID,
            endpoint_fingerprint="cassandra-attested",
            granted_privileges=["SELECT", "MODIFY", "CREATE", "ALTER"],
            missing_privileges=[],

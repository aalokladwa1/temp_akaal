"""
akaalEngine.connection.providers.streaming.kinesis
==================================================
Canonical AWS Kinesis Streams Provider Strategy.
Supports boto3 / kinesis client, shard iterators, and record streaming.
"""
...
logger = logging.getLogger("akaalEngine.connection.providers.kinesis")


class KinesisProviderStrategy(BaseProviderStrategy):
    """Canonical AWS Kinesis provider strategy."""
    ...
    def get_static_manifest(self) -> StaticCapabilityManifest:
        return StaticCapabilityManifest(
            ...
            capabilities={
                ...
                "SHARD_MANAGEMENT": CapabilitySupportStatus.SUPPORTED,
            },
    ...
    def connect(
        ...
        if not avail:
            raise DependencyMissingError(
                ConnectionFailure(
                    error_code="KINESIS_DEPENDENCY_MISSING",
    ...
    def attest_physical_identity(
        ...
        return PhysicalEndpointIdentity(
            ...
            provider_version=self.PROVIDER_VERSION,
            role=spec.role,
            resolved_host="kinesis.amazonaws.com",
            resolved_ip=resolved_route.resolved_ip,
            resolved_port=443,
            server_version="AWS Kinesis Managed Streams",
            catalog_or_database=spec.database_name or "stream",
            cloud_region=spec.region,
            route_type=spec.route_spec.route_type,
            topology_role="MANAGED_STREAM",
        )
    ...
    def probe_capabilities(
        ...
        return ProbedCapabilitySnapshot(
            provider_id=self.PROVIDER_ID,
            endpoint_fingerprint="kinesis-attested",
            capabilities={
                "STREAM_DISCOVERY": CapabilitySupportStatus.SUPPORTED,
                "STREAMING_READ": CapabilitySupportStatus.SUPPORTED,
                "STREAMING_WRITE": CapabilitySupportStatus.SUPPORTED,
    ...
    def probe_permissions(
        ...
        return PermissionSnapshot(
            provider_id=self.PROVIDER_ID,
            endpoint_fingerprint="kinesis-attested",
            granted_privileges=["kinesis:GetRecords", "kinesis:PutRecords", "kinesis:DescribeStream"],
            missing_privileges=[],

"""
tests.unit.engine_validation.test_p7a_campaign_b_remaining10_validation
======================================================================
P7A Campaign B — Remaining-10-Provider validation acceptance closure.
...
tests/unit/engine_transport/test_p7a_campaign_b_remaining10_transport_dataplane.py).
...
PROVIDER_ROW_SHAPES = {
    # Relational DB-API family (Teradata/Vertica/SAP HANA reuse GenericSQLSourceReader
    # directly; SAP ASE/Informix use the standalone unquoted-identifier reader) --
    # all produce the same lower-cased-column dict-of-tuples row shape.
    "teradata": [{"id": 1, "name": "alice"}, {"id": 2, "name": "bob"}],
    "vertica": [{"id": 1, "name": "alice"}, {"id": 2, "name": "bob"}],
    "sap_hana": [{"id": 1, "name": "alice"}, {"id": 2, "name": "bob"}],
    "sap_ase": [{"id": 1, "name": "alice"}, {"id": 2, "name": "bob"}],
    "informix": [{"id": 1, "name": "alice"}, {"id": 2, "name": "bob"}],
    "spanner": [{"id": 1, "name": "alice"}, {"id": 2, "name": "bob"}],
    # Cosmos DB: real document shape (arbitrary JSON fields plus the real "id" identity).
    "cosmosdb": [{"id": "x1", "name": "alice"}, {"id": "x2", "name": "bob"}],
    # Salesforce: real SObject shape (attributes envelope already stripped by the reader).
    "salesforce": [{"Id": "001x1", "Name": "alice"}, {"Id": "001x2", "Name": "bob"}],
    # ServiceNow: real Table API record shape (sys_id is the real record identity).
    "servicenow": [{"sys_id": "s1", "short_description": "alice"}, {"sys_id": "s2", "short_description": "bob"}],
    # SAP Application Ecosystem (OData mode): real OData entity-set record shape.
    "sap_application": [{"Id": "e1", "Name": "alice"}, {"Id": "e2", "Name": "bob"}],
}

PK_COLUMNS = {
    "teradata": ["id"], "vertica": ["id"], "sap_hana": ["id"], "sap_ase": ["id"], "informix": ["id"],
    "spanner": ["id"], "cosmosdb": ["id"], "salesforce": ["Id"], "servicenow": ["sys_id"],
    "sap_application": ["Id"],
}
...
def test_identical_source_target_rows_pass_exact_full_validation(provider_id):
    ...
    mismatches, for all 9 providers."""
...
def test_genuine_row_value_mismatch_is_detected_not_silently_passed(provider_id):
    ...
    detected as a real mismatch (not silently passed), for all 9 providers -- proving
...
def test_genuine_missing_row_is_detected_as_cardinality_mismatch(provider_id):
    ...
    genuine cardinality/missing-row mismatch, for all 9 providers."""
    val = ValidationAuthority()
    source_rows = PROVIDER_ROW_SHAPES[provider_id]

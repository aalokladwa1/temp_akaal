"""
tests.unit.engine_extensions.test_p7a_campaign_b_remaining10_certification
======================================================================
P7A Campaign B — Remaining-10-Provider certification / negative-capability acceptance
closure.
...
providers, proving:
  ...
  2. Every capability a provider declares UNsupported is genuinely rejected by
     ...
     capability enforcement mechanism -- for all 9 providers, closing the "negative
  ...
  3. Certification cannot self-certify: a provider cannot elevate its own capability
     ...
     remaining-10 provider strategies rather than synthetic test doubles.
"""
...
def test_real_certification_run_passes_contract_conformance(provider_id):
    ...
    every one of the 9 providers' live "connection" strategy instance -- contract
...
def test_every_declared_unsupported_capability_is_genuinely_enforced(provider_id):
    """For every one of the 9 providers, every capability its real resolved strategy
    handle declares UNsupported must be genuinely rejected by require_capability() --
    proving the negative-capability matrix is enforced in executable code for all 9,
...
def test_every_declared_supported_capability_resolves_truthfully(provider_id):
    ...
    declaration-integrity check, for all 9 providers."""
    ext_auth = _fresh_extensions_authority()
    report = ConnectorCertificationRunner.certify(ext_auth, provider_id, "connection")
...
def test_certification_obligations_reflect_only_actually_declared_capabilities(provider_id):
    ...
    provider's live strategy actually advertises (supported or not), for all 9
    providers."""
...
@pytest.mark.parametrize("provider_id", NEW_PROVIDERS)
def test_real_discovery_authority_certification_also_passes(provider_id):
    """Supplementary to the "connection" authority certification above: the real
    ConnectorCertificationRunner must also successfully certify each of the 9 providers'
    live "discovery" strategy instance (contract conformance + negative capability
    enforcement), proving certification coverage is not limited to a single authority."""
    ext_auth = _fresh_extensions_authority()
    from akaalEngine.discovery.authority import DiscoveryAuthority
    DiscoveryAuthority(extensions_authority=ext_auth)  # ensures discovery strategies bootstrapped

    report = ConnectorCertificationRunner.certify(ext_auth, provider_id, "discovery")
    assert report.provider_id == provider_id
    contract_check = next(r for r in report.results if r.check_name == "contract_conformance")
    assert contract_check.passed, f"'{provider_id}' failed real discovery-authority contract conformance: {contract_check.diagnostic}"

    negative_checks = [r for r in report.results if r.check_name == "negative_capability_enforced"]
    unenforced = [c for c in negative_checks if not c.passed]
    assert not unenforced, f"'{provider_id}' discovery authority has unenforced negative capabilities: {unenforced}"

...
class TestRealPostgresToMysql(unittest.TestCase):
    ...
    def test_postgres_to_mysql_migration(self):
        ...
        reset_source_database("postgres", POSTGRES_CONFIG)
        reset_target_database("mysql", MYSQL_CONFIG)
        ...
        load_schema("postgres", POSTGRES_CONFIG, schema_path)
        load_seed_data("postgres", POSTGRES_CONFIG, data_path)
        ...
        config = MigrationConfig(
            ...
            workspace_dir="./smoke_test_workspace_rev",
            project_name="PostgreSQL to MySQL Smoke Test",
            auto_approve=True,
        ...
        # A. Table count
        validate_table_count(conn_mysql, "mysql", 5)
        ...
        for table in tables:
            validate_row_count(conn_mysql, "mysql", table, expected_counts[table])
            validate_constraints(conn_pg, conn_mysql, ("postgres", "mysql"), table)
            validate_indexes(conn_pg, conn_mysql, ("postgres", "mysql"), table)
            validate_foreign_keys(conn_mysql, "mysql", table)
            validate_data_integrity(conn_pg, conn_mysql, ("postgres", "mysql"), table)
            
        # C. Specialized column validation
        validate_json_columns(conn_pg, conn_mysql, ("postgres", "mysql"), "products", "attributes")
        validate_json_columns(conn_pg, conn_mysql, ("postgres", "mysql"), "audit_logs", "new_value")
        validate_blob_columns(conn_pg, conn_mysql, ("postgres", "mysql"), "audit_logs", "raw_payload")
        ...
        print("\n" + "="*50)
        print("PERFORMANCE BENCHMARK SUMMARY (PostgreSQL -> MySQL)")
        print("="*50)

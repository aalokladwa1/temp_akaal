"""
Automated Architecture Conformance Test for Platform 4 CDC.

Asserts that no code in akaal/cdc/ imports internal engine or storage modules of other platforms directly.
"""
...
def test_enforce_platform4_boundary_isolation():
    root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "akaal", "cdc"))
    py_files = glob.glob(os.path.join(root_dir, "**", "*.py"), recursive=True)

    assert len(py_files) > 0, "No akaal/cdc Python files found to inspect"
    ...
    assert len(violations) == 0, f"Platform 4 Boundary Isolation Violations:\n" + "\n".join(violations)

"""
tests.security.test_p7a_campaign_b_remaining10_tenant_isolation
======================================================================
P7A Campaign B — Remaining-10-Provider tenant-isolation / security acceptance closure.

Proves, for the nine implemented remaining-10 providers, that the REAL, pre-existing (Campaign
...
SourceReader.open_partition() call, for every one of the 9 providers, using nothing
...
a physical read/write boundary for ANY of the 9 providers under a context claiming
...
def _make_keystore():
    ...
    ks = KeyStoreAuthority(keyring_repo=uow.keyring, master_root_key=b"mrk-32byte-test-remain10-tenant1")
...
def _make_durability(storage_dir):
    secret = "akaal-remaining10-tenant-isolation-secret-v1"
...
def test_matching_tenant_authorization_reaches_physical_boundary(provider_id, monkeypatch):
    ...
    call for every one of the 9 providers -- proving the barrier genuinely discriminates

...
======================================================================
P7A Campaign B — First-10-Provider Pipeline / ExecutionPlan acceptance closure.

Proves, for all 10 first-Campaign-B providers, that the REAL production
...
def test_compiled_plan_fingerprints_are_unique_across_all_10_providers():
    ...
    must yield 10 genuinely distinct canonical fingerprints."""
    fingerprints = set()
...
def test_dangling_edge_reference_rejected_regardless_of_provider(provider_id):
    ...
    but must hold for every provider's compiled plan, not just the originally-tested 28."""
    task = NodeTaskDescriptor(

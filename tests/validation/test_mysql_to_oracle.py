...
class TestMysqlToOracle(unittest.IsolatedAsyncioTestCase):

    async def asyncSetUp(self):
        try:
            self.conn_src = get_connection("mysql")
    ...
    async def test_mysql_to_oracle_migration(self):
        # 1. Reset both databases
        reset_source_database("mysql")
        reset_target_database("oracle_tgt")
        
        # 2. Apply schema and seed to source
        apply_schema("mysql")
        apply_seed_data("mysql")
        ...
        config = MigrationConfig(
            ...
            project_name="MySQL to Oracle Validation",
            auto_approve=True,
        ...
        for table in tables:
            ...
            validate_constraints(self.conn_src, self.conn_tgt, ("mysql", "oracle_tgt"), table)
            validate_indexes(self.conn_src, self.conn_tgt, ("mysql", "oracle_tgt"), table)
            validate_foreign_keys(self.conn_tgt, "oracle_tgt", table)
            validate_data_integrity(self.conn_src, self.conn_tgt, ("mysql", "oracle_tgt"), table)
            
        validate_json_columns(self.conn_src, self.conn_tgt, ("mysql", "oracle_tgt"), "products", "attributes")
        validate_json_columns(self.conn_src, self.conn_tgt, ("mysql", "oracle_tgt"), "audit_logs", "new_value")
        validate_blob_columns(self.conn_src, self.conn_tgt, ("mysql", "oracle_tgt"), "audit_logs", "raw_payload")
        ...
        print("\n" + "="*50)
        print("MIGRATION SMOKE TEST RESULT: MySQL -> Oracle")
        print("="*50)

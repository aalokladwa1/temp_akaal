...
class TestMysqlToPostgres(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_mysql_to_postgres_migration(self):
        ...
        config = MigrationConfig(
            ...
            project_name="MySQL to PostgreSQL Validation",
            auto_approve=True,
        ...
        for table in tables:
            ...
            validate_constraints(self.conn_src, self.conn_tgt, ("mysql", "postgres"), table)
            validate_indexes(self.conn_src, self.conn_tgt, ("mysql", "postgres"), table)
            validate_foreign_keys(self.conn_tgt, "postgres", table)
            validate_data_integrity(self.conn_src, self.conn_tgt, ("mysql", "postgres"), table)
            
        validate_json_columns(self.conn_src, self.conn_tgt, ("mysql", "postgres"), "products", "attributes")
        validate_json_columns(self.conn_src, self.conn_tgt, ("mysql", "postgres"), "audit_logs", "new_value")
        validate_blob_columns(self.conn_src, self.conn_tgt, ("mysql", "postgres"), "audit_logs", "raw_payload")
        ...
        print("\n" + "="*50)
        print("MIGRATION SMOKE TEST RESULT: MySQL -> PostgreSQL")
        print("="*50)

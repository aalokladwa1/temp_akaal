...
class TestPostgresToOracle(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_postgres_to_oracle_migration(self):
        ...
        config = MigrationConfig(
            ...
            project_name="PostgreSQL to Oracle Validation",
            auto_approve=True,
        ...
        for table in tables:
            ...
            validate_constraints(self.conn_src, self.conn_tgt, ("postgres", "oracle_tgt"), table)
            validate_indexes(self.conn_src, self.conn_tgt, ("postgres", "oracle_tgt"), table)
            validate_foreign_keys(self.conn_tgt, "oracle_tgt", table)
            validate_data_integrity(self.conn_src, self.conn_tgt, ("postgres", "oracle_tgt"), table)
            
        validate_json_columns(self.conn_src, self.conn_tgt, ("postgres", "oracle_tgt"), "products", "attributes")
        validate_json_columns(self.conn_src, self.conn_tgt, ("postgres", "oracle_tgt"), "audit_logs", "new_value")
        validate_blob_columns(self.conn_src, self.conn_tgt, ("postgres", "oracle_tgt"), "audit_logs", "raw_payload")
        ...
        print("\n" + "="*50)
        print("MIGRATION SMOKE TEST RESULT: PostgreSQL -> Oracle")
        print("="*50)

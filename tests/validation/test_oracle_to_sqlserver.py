...
class TestOracleToSqlserver(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_oracle_to_sqlserver_migration(self):
        ...
        config = MigrationConfig(
            ...
            project_name="Oracle to SQL Server Validation",
            auto_approve=True,
        ...
        for table in tables:
            ...
            validate_constraints(self.conn_src, self.conn_tgt, ("oracle_src", "sqlserver"), table)
            validate_indexes(self.conn_src, self.conn_tgt, ("oracle_src", "sqlserver"), table)
            validate_foreign_keys(self.conn_tgt, "sqlserver", table)
            validate_data_integrity(self.conn_src, self.conn_tgt, ("oracle_src", "sqlserver"), table)
            
        validate_json_columns(self.conn_src, self.conn_tgt, ("oracle_src", "sqlserver"), "products", "attributes")
        validate_json_columns(self.conn_src, self.conn_tgt, ("oracle_src", "sqlserver"), "audit_logs", "new_value")
        validate_blob_columns(self.conn_src, self.conn_tgt, ("oracle_src", "sqlserver"), "audit_logs", "raw_payload")
        ...
        print("\n" + "="*50)
        print("MIGRATION SMOKE TEST RESULT: Oracle -> SQL Server")
        print("="*50)

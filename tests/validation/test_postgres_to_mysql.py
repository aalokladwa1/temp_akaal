...
class TestPostgresToMysql(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_postgres_to_mysql_migration(self):
        ...
        config = MigrationConfig(
            ...
            project_name="PostgreSQL to MySQL Validation",
            auto_approve=True,
        ...
        for table in tables:
            ...
            validate_constraints(self.conn_src, self.conn_tgt, ("postgres", "mysql"), table)
            validate_indexes(self.conn_src, self.conn_tgt, ("postgres", "mysql"), table)
            validate_foreign_keys(self.conn_tgt, "mysql", table)
            validate_data_integrity(self.conn_src, self.conn_tgt, ("postgres", "mysql"), table)
            
        validate_json_columns(self.conn_src, self.conn_tgt, ("postgres", "mysql"), "products", "attributes")
        validate_json_columns(self.conn_src, self.conn_tgt, ("postgres", "mysql"), "audit_logs", "new_value")
        validate_blob_columns(self.conn_src, self.conn_tgt, ("postgres", "mysql"), "audit_logs", "raw_payload")
        ...
        print("\n" + "="*50)
        print("MIGRATION SMOKE TEST RESULT: PostgreSQL -> MySQL")
        print("="*50)
